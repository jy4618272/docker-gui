 ...
 struct p_link_image
{
 ...
 struct p_link_image image_list[1024]={};
...
void add_data_combox(GtkWidget *p_combox)//往combox添加镜像的数据
{
	...
	memset(p_buf,'\0',1*sizeof(p_buf));
	p_store=gtk_list_store_new (2,G_TYPE_STRING,G_TYPE_STRING);
	GtkCellRenderer *p_cell,*p_cell2;
	p_cell=gtk_cell_renderer_text_new ();
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(p_combox),p_cell,TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(p_combox),p_cell,"text",0,NULL);
	p_cell2=gtk_cell_renderer_text_new ();
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(p_combox),p_cell2,TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(p_combox),p_cell2,"text",1,NULL);
	...
	gtk_combo_box_set_model (GTK_COMBO_BOX(p_combox),GTK_TREE_MODEL(p_store));
	pclose(p_fd);
	g_free (p_buf);
...
void p_combx_changed(GtkWidget *window,gpointer data)//获取combox每次选择的数据
{
	...
	model=gtk_combo_box_get_model (GTK_COMBO_BOX(window));
	gtk_combo_box_get_active_iter (GTK_COMBO_BOX(window),&iter_p);
...
gpointer p_link_fun(gpointer data)//线程函数
{
	//gdk_threads_enter ();
//printf("%s\n",data);
...
{
		const char *p_link_s=NULL;
		const  char *p_link_entry_data=NULL;
		const  char *p_link_contains_data=NULL;
		...
		memset((char *)p_link_s,'\0',1*sizeof(p_link_s));
		p_link_entry_data=gtk_entry_get_text (GTK_ENTRY(p_entry));
		p_link_contains_data=gtk_entry_get_text (GTK_ENTRY(p_entry_con));						
		
		if(*p_link_entry_data==0){
			contains_message(GTK_MESSAGE_ERROR,_("Port number can not be empty"));
			
				return;
		}
		if(*p_link_contains_data==0){
		...
		if(p_link_combo_data[0]=='\0')
	{
		...
		sprintf((char *)p_link_s,"sudo gnome-terminal -x bash -c \"sudo docker run -ti --rm --name %s %s %s /bin/bash\"",
		        (char *)p_link_contains_data,p_link_entry_data,p_link_combo_data);
			printf("%s\n",p_link_s);
			//g_free (p_link_entry_data);
			//g_free (p_link_contains_data);
			g_thread_new ("p_link",p_link_fun,(char *)p_link_s);
