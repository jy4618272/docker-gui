...
#define BACKUP_TAR_LOCAL_PATH "/root/.nadt/volume_tar"  /* 存放tar包(备份数据卷)的本地目录 */
#define BACKUP_TAR_DOCKER_PATH "/transfer"        /* 备份时,将tar包目录挂载到容器之中的目录 */
#define DOCKER_LOCAL_PATH "/var/lib/docker/containers"
#define BACKUP_TAR_INFO "/root/.nadt/volume_tar/info.txt"

#ifdef DEBUG
#define BACKUP_VOLUME_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_volume_backup.ui"
#define VOLUME_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_volume.ui"
#define CREATE_VOLUMN_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_volume_create.ui"
#define CREATE_CONTAINER_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_volume_container_create.ui"
#define LOOK_VOLUME_FILE  UI_DIR_PATH_FOR_DEBUG"/nfs_volume_look.ui"
#define TRANSFER_VOLUME_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_volume_transfer.ui"
#else
#define BACKUP_VOLUME_FILE UI_DIR_PATH_FOR_BUILD"/nfs_volume_backup.ui"
#define VOLUME_FILE UI_DIR_PATH_FOR_BUILD"/nfs_volume.ui"
#define CREATE_VOLUMN_FILE UI_DIR_PATH_FOR_BUILD"/nfs_volume_create.ui"
#define CREATE_CONTAINER_FILE UI_DIR_PATH_FOR_BUILD"/nfs_volume_container_create.ui"
#define LOOK_VOLUME_FILE  UI_DIR_PATH_FOR_BUILD"/nfs_volume_look.ui"
#define TRANSFER_VOLUME_FILE UI_DIR_PATH_FOR_BUILD"/nfs_volume_transfer.ui"
#endif
...
{	
	...
	if(NULL == opendir("/root/.nadt"))
	{
        mkdir("/root/.nadt", 0755);
		mkdir("/root/.nadt/volume_tar", 0755);
    }
	else if(NULL == opendir("/root/.nadt/volume_tar"))
		mkdir("/root/.nadt/volume_tar", 0755);
...
void data_volume_look_add_data(GtkBuilder *builder, gint flag)
{
    ...
    if(pDir == NULL)
    {
		message_dialog_new (NULL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, _("path open failed"));
        return ;
    }
...
//	gtk_button_set_label (GTK_BUTTON(button_look), _("lookup the datavolume you have backuped"));
	...
	gtk_button_set_label (GTK_BUTTON(button1),_("selete default path"));
	gtk_button_set_label (GTK_BUTTON(button2),_("change default path"));
	gtk_label_set_text (GTK_LABEL(label5),_("look for path"));
	gtk_widget_set_tooltip_markup (label5, "<span foreground=\"#000000\">/root/.nadt/volume_tar</span>");
	...
	g_signal_connect(G_OBJECT(button1),"clicked",G_CALLBACK(data_volume_backup_button1_clicked),label5);
	g_signal_connect(G_OBJECT(button2),"clicked",G_CALLBACK(data_volume_backup_button2_clicked),label5);
...
{
	gtk_widget_set_tooltip_markup (GTK_WIDGET(label), "<span foreground=\"#000000\">/root/.nadt/volume_tar</span>");
	backup_flag = 1;
}
...
void data_volume_backup_button2_clicked(GtkWidget *widget, GtkLabel *label)
{
	...
	tar_name = (gchar *)g_malloc(sizeof(gchar)*1024);
	text     = (gchar *)g_malloc(sizeof(gchar)*1024);
	
	file_chooser = gtk_file_chooser_dialog_new(_("please choose the path you want to backup"),
	                                           NULL, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("cancel"),
	...
	gtk_window_set_default_size (GTK_WINDOW(file_chooser), 600, 500);
	
	if(result == GTK_RESPONSE_OK)
	{   
		tar_name = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER(file_chooser));
		sprintf(text, "<span foreground=\"#000000\">%s</span>", tar_name);
		gtk_widget_set_tooltip_markup (GTK_WIDGET(label), text);
...
void data_volume_backup_ok(GtkWidget *widget, GtkBuilder *builder)
{
	...
	else if(backup_flag == 2)
	{
		GtkWidget *path_label = GTK_WIDGET(gtk_builder_get_object (builder, "label5"));
		path = gtk_widget_get_tooltip_text(path_label);
		sprintf(str,"%s -v %s:%s",str, path, BACKUP_TAR_DOCKER_PATH);
	}
	else
		return;
	...
	data_volume_backup_write_info_to_txt(builder, data_name, path);
...
void data_volume_backup_pick_up_data(gchar *str, gchar *data, gint flag)
{
	...
	{
		sprintf(command, "echo %s | awk -F : '{for(i=1;i<NF;i++) printf $i\" \"}'"
	    			             "| awk '{for(i=1;i<=NF;i+2){printf $i\" \"}}'", data_tmp);
	}
...
gint data_volume_backup_get_name(GtkBuilder *builder, gchar *str)
{
	...
	if(RETURN_OK != (data_volume_backup_check_name (builder, entry_text)))
		return NO_NAMES;
...
gint data_volume_backup_check_name(GtkBuilder *builder, const gchar *str)
{
	...
	path_label = GTK_WIDGET(gtk_builder_get_object (builder, "label5"));
	path = gtk_widget_get_tooltip_text(path_label);
	...
	if(backup_flag == 1)
		...
		sprintf(command, "find %s/*.tar | awk 'END{print NR}'",path);
	else
		return -1;
	...
	for(ix=1; ix<=num; ix++)
	{
		...
		if(backup_flag == 1)
			...
			sprintf(command, "find %s/*.tar | sed -n \"%d,1p\" | awk -F / '{print $NF}'",path, ix);
		else
			return -1;
...
void data_volume_backup_write_info_to_txt(GtkBuilder *builder, gchar *str, gchar *path)
{
	...
	if(backup_flag == 2)
		sprintf(msg, "%s/%s.tar : %s\n", path, entry_text, str);
	else
		sprintf(msg, "%s.tar : %s\n", entry_text, str);

	if(NULL == (fp = fopen(BACKUP_TAR_INFO, "a")))
	{
...
//	GtkWidget *button         = GTK_WIDGET(gtk_builder_get_object (builder, "button")); 
	GtkWidget *buttonbox         = GTK_WIDGET(gtk_builder_get_object (builder, "buttonbox3")); 
...
//	gtk_button_set_label (GTK_BUTTON(button), _("look up the datavolume you have recoveried"));
	...
	if(NULL == (fp = fopen(BACKUP_TAR_INFO, "r")))
		message_dialog_new (NULL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, _("the default path has none datavolume to recovery,"
...
void data_volume_recovery_add_data(GtkBuilder *builder, gboolean flag)
{
	...
	sprintf(str,"cat %s | awk 'END{print NR}'", BACKUP_TAR_INFO);
	strcpy(result, popen_and_fread (str));
	lines  = atoi(result);
	...
	for(ix=0; ix<lines; ix++)
	{
		...
		sprintf(str, "cat %s | sed -n \"%d,1p\"", BACKUP_TAR_INFO, ix+1);
		strcpy(result, popen_and_fread (str));
		sscanf(result,"%s : %[/0-9a-z ]",tar_str,data_str);
...
void data_volume_recovery_ok(GtkWidget *widget, GtkBuilder *builder)
{
	...
	*/
	...
	data_volume_recovery_get_tar_name(builder, tar_name, NULL);

	sprintf(str1, "sudo docker run --rm --volumes-from %s ", entry_name); 
	
	if(data_volume_recovery_data(str1, tar_name, image_name))
	{
...
void data_volume_recovery_get_tar_name(GtkBuilder *builder, gchar  *tar_name, gchar *num)
{
	...
	{		
		do
		{
			gtk_tree_model_get(model, &iter, 6, &flg, 1, &tar_name_tmp, 0, &num_tmp, -1);
			if(flg)
			{
				if(TRANSFER_DELETE_FLAG)
					sprintf(num,"%s %d", num, num_tmp);
...
gboolean 	data_volume_recovery_data(gchar *str, gchar *tar_name, gchar *image)
{
	...
	while(pData)
	{
		char pData_tmp[512] = {0};
		char *p = NULL;
		strcpy(pData_tmp, pData);
		...
		if(p)
		{
			int len = strlen(p);
			char path[512] = {0};
			strncpy(path, pData_tmp, strlen(pData_tmp)-len);
		
			sprintf(str_tmp, "%s -v %s:%s %s tar xvf %s%s", str, path,
			    							BACKUP_TAR_DOCKER_PATH, image,
			    							BACKUP_TAR_DOCKER_PATH, p);
		}

		else
		{
			sprintf(str_tmp, "%s -v %s:%s %s tar xvf %s/%s", str,BACKUP_TAR_LOCAL_PATH, 
	    									BACKUP_TAR_DOCKER_PATH, image,
	    									BACKUP_TAR_DOCKER_PATH, pData_tmp); 
		}
...
void data_volume_transfer(GtkWidget *widget, GtkWidget *window)
{
	...
	if (!gtk_builder_add_from_file (builder, TRANSFER_VOLUME_FILE, &error))
	{
		g_critical ("Couldn't load builder file: %s", error->message);
		g_error_free (error);
	}
...
//	GtkWidget *button         = GTK_WIDGET(gtk_builder_get_object (builder, "button")); 
	...
	gtk_label_set_text(GTK_LABEL(label4), _("input the name of new container"));
	gtk_label_set_text (GTK_LABEL(label7),_("please input the local dir name for transfer the data volume"));
	gtk_button_set_label (GTK_BUTTON(button1),_("transfer to default path"));
	gtk_button_set_label (GTK_BUTTON(button2),_("change default path"));
	gtk_button_set_label (GTK_BUTTON(button3),_("delete the tar you have choosen"));
	gtk_label_set_text (GTK_LABEL(label5),_("look for path"));
	gtk_widget_set_tooltip_markup (label5, _("<span foreground=\"#000000\">/transfer</span>"));
	gtk_widget_set_tooltip_markup (button1, _("<span foreground=\"#000000\">the absolute path in the container(/transfer)</span>"));
	gtk_widget_set_tooltip_markup (button2, _("<span foreground=\"#000000\">the absolute path in the container(for exampl: /mytest)</span>"));
//	gtk_button_set_label (GTK_BUTTON(button), _("look up the datavolume that have transfered"));
	...
	g_signal_connect(G_OBJECT(button1),"clicked",G_CALLBACK(data_volume_transfer_button1_clicked),builder);
	g_signal_connect(G_OBJECT(button2),"clicked",G_CALLBACK(data_volume_transfer_button2_clicked),builder);
	g_signal_connect(G_OBJECT(button3),"clicked",G_CALLBACK(data_volume_transfer_button3_clicked),builder);
	g_signal_connect(G_OBJECT(check_button),"clicked",G_CALLBACK(data_volume_recovery_select_all),builder);
	...
	if(NULL == (fp = fopen(BACKUP_TAR_INFO, "r")))
		message_dialog_new (NULL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, _("the default path has none datavolume to transfer,"
		                                                                 "you could change the path"));
}
...
void data_volume_transfer_button3_clicked(GtkWidget *widget, GtkBuilder *builder)
{
	...
	if(!(label_text && *label_text))
		message_dialog_new (NULL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, _("please choose the tar you want to delete first!"));
	else
	{
		GtkWidget *dialog = gtk_message_dialog_new(NULL,GTK_DIALOG_MODAL| 
	             		GTK_DIALOG_DESTROY_WITH_PARENT,GTK_MESSAGE_INFO,
	                    GTK_BUTTONS_YES_NO, _("make suue to delete?"));
		gint conditions = gtk_dialog_run(GTK_DIALOG(dialog));
		
		switch(conditions)
		{
			case GTK_RESPONSE_YES:  /* 进入到创建的新容器 */
				data_volume_transfer_button3_response_ok(builder);
				gtk_widget_destroy (dialog);
				break;
...
{
	gchar tar_name[1024] = {0};
	gchar command[1024] = "sudo rm -rf ";
	gchar num_command[256] = {0};
	gchar *pData   = NULL;
	gchar str[1024] = {0};
	gchar *result = NULL;
	gint num = 0;

	TRANSFER_DELETE_FLAG = TRUE;
	data_volume_recovery_get_tar_name(builder, tar_name, num_command);
	TRANSFER_DELETE_FLAG = FALSE;
	
	pData = strtok(tar_name," ");
	...
	while(pData)
	{
		...
		p = (strchr(pData_tmp,'/'));
		if(p)
			sprintf(command, "%s %s", command, pData_tmp);

		else
			sprintf(command, "%s /root/.nadt/volume_tar/%s", command, pData_tmp);
	}

	strrev(num_command);

	result = strtok(num_command, " ");
	num = atoi(result);
	sprintf(str, "sed -i '%dd' /root/.nadt/volume_tar/info.txt", num);
	popen_and_fread (str);
...
char *strrev(char *s)
{
	char *head = s; /* head points to the head of s */
	char *tail = s;
	char ch;

	while(*tail++){} /* tail points to the tail of s */
	tail--;
	tail--;

	while(head < tail)/* swap the chars */
	{
		ch = *head;
		*head++ = *tail;
		*tail-- = ch;
...
void data_volume_transfer_ok(GtkWidget *widget, GtkBuilder *builder)
{
	...
	gchar image_name[128] = {0};
	gchar entry_name[128] = {0};
	gchar container_dir_name[512] = {0};
	gchar local_dir_name[512] = {0};
	...
	memset(entry_name, '\0', sizeof(entry_name));
	memset(tar_name, '\0', sizeof(tar_name));
	memset(local_dir_name, '\0', sizeof(local_dir_name));
	memset(container_dir_name, '\0', sizeof(container_dir_name));
	...
	/* 判断是否输入新目录的名称 */
	if(RETURN_OK != data_volume_transfer_get_local_dir_name(builder, local_dir_name))
			return;

	/* 判断是否输入新建容器存放迁移数据卷目录的名称  */
	if(transfer_flag == 1)
		strcpy(container_dir_name, BACKUP_TAR_DOCKER_PATH);
	if(transfer_flag == 2)
	{
		if(RETURN_OK != data_volume_transfer_get_container_dir_name(builder, container_dir_name))
			return;
	}
	...
	/*  根据目录名称新建目录 */
	if(!data_volume_transfer_create_directory(local_dir_name, tar_name))
		return;
	
	sprintf(str, "sudo docker run -idt --name %s -v %s/%s:%s %s ",
	    						entry_name, BACKUP_TAR_LOCAL_PATH, local_dir_name, 
	    						container_dir_name, image_name);
...
gint data_volume_transfer_get_local_dir_name(GtkBuilder *builder, gchar *str)
{
	...
	entry = GTK_ENTRY(gtk_builder_get_object (builder, "entry3"));
	entry_text = gtk_entry_get_text (entry);

	if(!(entry_text && *entry_text))
	{
		message_dialog_new (NULL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _("please input the name of local directory!!!"));
		return NO_CONTAINER;
	}
...
gint data_volume_transfer_get_container_dir_name(GtkBuilder *builder, gchar *str)
{
	...
	if(!(entry_text && *entry_text))
	{
		message_dialog_new (NULL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _("please input the absolute path of new container!"));
		return NO_CONTAINER;
	}
...
gboolean data_volume_transfer_create_directory(gchar *dir_name, gchar *tar_name)
{
	...
	while(pData)
	{
		sprintf(str, "%s %s",str, pData);
		pData = strtok(NULL, " ");
	}
	/* 创建目录 */
	sprintf(str1,"cd /root/.nadt/volume_tar && sudo mkdir %s", dir_name); 
	if(data_volume_recovery_create_container(str1))
	{
		memset(str1, '\0', sizeof(str1));
		
		/* 讲选择的相关tar包进行解压 */
		sprintf(str1,"cd /root/.nadt/volume_tar && sudo cp %s %s"
	    			     "&& cd %s && sudo ls *.tar |sudo xargs -n1 tar xvf && sudo rm -rf *.tar",
