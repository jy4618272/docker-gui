...
button22_fun(GtkWidget *button,gpointer *data)//输入命令的功能
{
		...
		command11=(gchar *)gtk_entry_get_text(GTK_ENTRY(data));
		...
		dialog11=gtk_message_dialog_new(NULL,GTK_DIALOG_MODAL|GTK_DIALOG_DESTROY_WITH_PARENT,type11,GTK_BUTTONS_OK,message_info,NULL);
...
button33_fun(GtkWidget *button,gpointer *data)//"关于Docker查看容器 查看版本"按钮的功能
{
	...
	if(pp == 0){
		if(data!=NULL){
			fd1=popen(error_fd,"r");
				if(fread(buf2,sizeof(char),sizeof(buf2),fd1))
	...
	dialog=gtk_message_dialog_new(NULL,GTK_DIALOG_MODAL|GTK_DIALOG_DESTROY_WITH_PARENT,type11,GTK_BUTTONS_OK,message_formart,NULL);
