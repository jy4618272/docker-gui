...
#ifdef DEBUG
#define IMAGE_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image.ui"
#define IMAGE_IMAGE_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image_image.ui"
#define SAVE_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image_save.ui"
#define LOAD_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image_save_load_rmi.ui"
#define RMI_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image_save_load_rmi.ui"
#define TAG_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image_save_load_rmi.ui"
#define PULL_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image_pull.ui"
#define COMMIT_FILE UI_DIR_PATH_FOR_DEBUG"/nfs_image_commit.ui"
#else
#define IMAGE_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image.ui"
#define IMAGE_IMAGE_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image_image.ui"
#define SAVE_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image_save.ui"
#define LOAD_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image_save_load_rmi.ui"
#define RMI_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image_save_load_rmi.ui"
#define TAG_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image_save_load_rmi.ui"
#define PULL_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image_pull.ui"
#define COMMIT_FILE UI_DIR_PATH_FOR_BUILD"/nfs_image_commit.ui"
#endif

#define LOAD_AND_SAVE_PATH  "/root/.nadt/image_tar"
...
static gint load_flag = 1;
static gint save_flag = 1;
static gint max_page = 0;
static gboolean pull_flag=TRUE;
static gint current_page = 0;
static gint tar_num = 0;
const  gchar *pull_text = NULL;
...
void image_event(GtkWidget *widget, GtkWidget *window)
{
	...
	if(NULL == opendir("/root/.nadt"))
	{
		...
		mkdir("/root/.nadt/image_tar", 0755);
	}
	else if(NULL == opendir("/root/.nadt/image_tar"))
		mkdir("/root/.nadt/image_tar", 0755);
	...
	g_signal_connect(G_OBJECT(build_button),"clicked", G_CALLBACK(select_dockerfile_textview), NULL);
...
//	gdk_threads_enter ();
	...
	pull_window = GTK_WIDGET(gtk_builder_get_object (builder,"pull_window"));
	entry       = GTK_WIDGET(gtk_builder_get_object (builder,"entry1"));
	pull_text   = gtk_entry_get_text (GTK_ENTRY(entry));
	...
	sprintf(str,"sudo docker pull %s 2>&1; echo $?",pull_text);
...
/*	while(fread(search_result, 1, sizeof(search_result), fp) != 0)
	{
		...
		{
			message_dialog_new (NULL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, 
		                    _("fread failed, please try again!"));
			pclose(fp);
			printf("eeeeeee\n");
			return;
		}
	} */
	while(fgets(search_result,64,fp) !=NULL)
	{
		if(fgets(search_result,64,fp))
			printf("%s\n",search_result);
		sleep(1);
	}
...
//		gdk_threads_leave ();
	...
	g_idle_add (message_dialog_create, _("download successfully，you could look for it outside"));
	g_idle_add(destroy_spinner , window);
	g_idle_add(set_window_true, pull_window);
...
{   
	...
	GtkWidget *buttonbox4 = GTK_WIDGET(gtk_builder_get_object (builder,"buttonbox4"));
...
void rmi_ok_clicked(GtkWidget *widget, GtkBuilder *builder)
{
    ...
    sprintf(str1,"sudo docker rmi -f %s 2>&1;echo $?",str);
...
void image_save_event(GtkWidget *button, GtkWidget *window)
{
	...
	label1        = GTK_WIDGET(gtk_builder_get_object (builder,"label1"));
	label2        = GTK_WIDGET(gtk_builder_get_object (builder,"label4"));
	...
	buttonbox4    = GTK_WIDGET(gtk_builder_get_object (builder,"buttonbox4"));
	...
	gtk_button_set_label (GTK_BUTTON(button3),_("selete default path"));
	gtk_button_set_label (GTK_BUTTON(button4),_("change default path"));
	gtk_label_set_text (GTK_LABEL(label),_("look for path"));
	gtk_widget_set_tooltip_markup (label, "<span foreground=\"#000000\">/root/.nadt/image_tar</span>");
	...
	g_signal_connect(G_OBJECT(button3),"clicked",G_CALLBACK(save_radio1_clicked),label);
	g_signal_connect(G_OBJECT(button4),"clicked",G_CALLBACK(save_radio2_clicked),label);
...
{
	gtk_widget_set_tooltip_markup (button, "<span foreground=\"#000000\">/root/.nadt/image_tar</span>");
	save_flag = 1;
}
...
void save_radio2_clicked(GtkWidget *radiobutton, GtkWidget *button)
{
	...
	file_chooser = gtk_file_chooser_dialog_new(_("please choose the image you want to load"),
...
{	
	...
	{
		gchar *path = NULL;
		GtkWidget *path_button = GTK_WIDGET(gtk_builder_get_object (labels->builder, "button2"));
		path = gtk_widget_get_tooltip_text(path_button);
		sprintf(str,"sudo docker save -o %s/%s.tar %s",path,text,tags);
		g_free(path);
	}
...
void image_load_event(GtkWidget *button, GtkWidget *window)
{
	...
	gtk_label_set_text (GTK_LABEL(label1),_("please choose the tar you want to load from default path"));
	gtk_label_set_text (GTK_LABEL(label3),_("the tar you have chosen: "));
	gtk_label_set_text (GTK_LABEL(label5),_("please select the tar "));
	gtk_label_set_justify (GTK_LABEL(label4),GTK_JUSTIFY_CENTER);
	gtk_label_set_max_width_chars(GTK_LABEL(label4), 50);
	gtk_button_set_label (GTK_BUTTON(button1), _("click to choose"));

	g_signal_connect(G_OBJECT(button1),"clicked",G_CALLBACK(load_file_button_select),label4);
...
//	sprintf(str,"find /root/.nadt/image_tar -name \"*.tar\" | awk 'END{print NR}'");
//	if(NULL == opendir("/root/.nadt/image_tar") || 0 == strcmp("0", popen_and_fread (str)))
	if(tar_num == 0)
		message_dialog_new (NULL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, _("the default path has no tar to load,"
		                                                                 "you could select the path to load!"));
}
...
void load_file_button_select(GtkButton *button, GtkLabel *label)
{
	...
	tar_name = (gchar *)g_malloc(sizeof(gchar)*512);
	text     = (gchar *)g_malloc(sizeof(gchar)*512);

	filter = gtk_file_filter_new ();
	file_chooser = gtk_file_chooser_dialog_new(_("please choose the image you want to load"),
	                                           NULL, GTK_FILE_CHOOSER_ACTION_OPEN, _("cancel"),
	                                           GTK_RESPONSE_CANCEL,_("ok"),GTK_RESPONSE_OK,NULL);
	...
	{   
		tar_name = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER(file_chooser));
		sprintf(text, "<span foreground=\"#000000\">%s</span>", tar_name);
		gtk_label_set_text (label,_("move cursor to me"));
...
//	sscanf(text,"%*[^:]:%s",tar);  
		...
		sprintf(str,"sudo docker load --input %s/%s 2>&1; echo $?",LOAD_AND_SAVE_PATH,text);
	}
	
	else if(load_flag == 2)
	{   
		text = gtk_widget_get_tooltip_text (GTK_WIDGET(label));
		sprintf(str,"sudo docker load --input %s 2>&1; echo $?",text);
	}
	else
		return;
...
void load_add_tar_item(GtkBuilder *builder)
{
	...
	{
		memset(str,'\0',sizeof(str));
		sprintf(str,"find %s -name \"*.tar\" | awk -F \"/\" '{print $NF}' | sed -n \"%d,1p\" ",LOAD_AND_SAVE_PATH,x+1); 
		memset(result,'\0',sizeof(result));
		strcpy(result,popen_and_fread (str));
...
void image_tag_event(GtkWidget *button, GtkWidget *window)
{
	...
	gtk_label_set_markup (GTK_LABEL(label_tip), _("<span background=\"#ffff00\">Note: if you have taged successfully, it will not cover the original image, but create a new image you have named</span> "));
...
void cancel_click(GtkWidget button, GtkWidget *window)
{
	gchar str[256] = {0};
	gchar *result = NULL;
	pull_flag = FALSE;
	
	sprintf(str, "ps aux | grep \"docker pull %s\" | awk '{printf$2\" \"}'", pull_text);
	result = popen_and_fread (str);

	memset(str, '\0', strlen(str));
	sprintf(str, "sudo kill -s 9 %s", result);
	popen_and_fread (str);
}
...
gboolean create_spinner(gpointer data)
{
	...
	label = gtk_label_new (_("pull slowly, please wait patiently"));
	button = gtk_button_new_with_label (_("stop to pull"));
	gtk_widget_set_size_request (GTK_WIDGET(spinner), 80, 80);
	gtk_container_add(GTK_CONTAINER(window), hbox);

	gtk_box_pack_start (GTK_BOX(hbox), GTK_WIDGET(spinner), TRUE, TRUE, 0);
	gtk_spinner_start (spinner);
	gtk_box_pack_start (GTK_BOX(hbox), vbox, TRUE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX(vbox), label, TRUE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX(vbox), button, TRUE, FALSE, 0);

	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(cancel_click), window);
