...
void button77_lsgurd_fun(GtkWidget *window,gpointer data)//从文件读取守护容器的信息
{
	...
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(image_nam),_("please select image name"));
	//gtk_tree_view_column_set_title (id,_("asdasd"));
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(id),_("gurd contain ID"));
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(nam),_("gurd contain name"));
	
	cell_image=gtk_cell_renderer_text_new ();
	gtk_tree_view_column_pack_start (GTK_TREE_VIEW_COLUMN(image_nam),cell_image,TRUE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(image_nam),cell_image,"text",0,NULL);

	cell_image=gtk_cell_renderer_text_new ();
	gtk_tree_view_column_pack_start (GTK_TREE_VIEW_COLUMN(id),cell_image,TRUE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(id),cell_image,"text",1,NULL);

	cell_image=gtk_cell_renderer_text_new ();
	gtk_tree_view_column_pack_start (GTK_TREE_VIEW_COLUMN(nam),cell_image,TRUE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(nam),cell_image,"text",2,NULL);
	...
	gtk_window_set_title (GTK_WINDOW(window_lsgurd22),_("ls gurd contain"));
...
{
	GtkWidget *dialog;
	dialog=gtk_message_dialog_new(NULL,GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,type,GTK_BUTTONS_OK,buf,NULL);
...
gpointer docker_contains_create_func(gpointer data)//创建系统默认名字容器的功能
{	
	
	if(system("sudo gnome-terminal -x bash -c \"sudo docker run -ti  ubuntu /bin/bash\""))
		g_print("suss\n");
	return (void* )1;
}
gpointer docker_tree_view_clean(gpointer data)//创建容器后刷新界面
{
	...
	gtk_list_store_clear (list11);
	for(j=0;j<contains_num;j++){
	gtk_list_store_append(list11,&iter11);
	gtk_list_store_set (GTK_LIST_STORE(list11),&iter11, 0,&a[j].image,-1);
	gtk_list_store_set (GTK_LIST_STORE(list11),&iter11, 1,&a[j].id,-1);
	gtk_list_store_set (GTK_LIST_STORE(list11),&iter11, 2,&a[j].name,-1);
		}
	return (void *)0;
}
...
void button33_create_gurd_contians_fun(GtkWidget *window,gpointer data)//守护界面按钮信号
{
	...
	gint num=0;
	gint num22=0;
	const gchar *bf1=NULL;
	const gchar *bf2=NULL;
	gchar *tmp11=NULL;
	tmp11=(char *)malloc(sizeof(char)*1024);
	bf1=gtk_entry_get_text (GTK_ENTRY(gurd_entry1));
	bf2=gtk_entry_get_text (GTK_ENTRY(gurd_entry2));
...
void combox_gurd_change(GtkWidget *window,gpointer data)//守护comb信号
{
	...
	model_combox=gtk_combo_box_get_model (GTK_COMBO_BOX(window));
	gtk_combo_box_get_active_iter (GTK_COMBO_BOX(window),&iter_combo);
...
void button11_create_contians_view_fun(GtkWidget *window,gpointer data)//判断创建容器的主界面填写的内容及创建容器
{
	...
	bf=gtk_entry_get_text (GTK_ENTRY(create_contians_view_entry22));
...
void combox_images_data(GtkWidget *combox_all)//所有combox获取镜像的信息
{
	...
	memset(images_buf,'\0',1*sizeof(images_buf));
	...
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(combox_all),cell_images,TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combox_all),cell_images,"text",0,NULL);
	GtkCellRenderer *cell_images22=gtk_cell_renderer_text_new ();
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(combox_all),cell_images22,FALSE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combox_all),cell_images22,"text",1,NULL);
	gtk_combo_box_set_model (GTK_COMBO_BOX(combox_all),GTK_TREE_MODEL(list_images));
	...
	void export_selcet_file(GtkWidget *window,gpointer data)
{
	...
	//dialog=gtk_file_chooser_dialog_new(_("Please rename the exported file"),NULL,GTK_FILE_CHOOSER_ACTION_SAVE,GTK_STOCK_CANCEL,GTK_RESPONSE_CANCEL,GTK_STOCK_SAVE,
	 //                                  GTK_RESPONSE_OK,NULL);
			...
			dialog=gtk_file_chooser_dialog_new(_("Please rename the exported file"),NULL,GTK_FILE_CHOOSER_ACTION_SAVE,_("取消"),GTK_RESPONSE_CANCEL,_("sure"),
	                                   GTK_RESPONSE_OK,NULL);
...
void import_create_file_veiw_fun(GtkWidget *window,gpointer data)
{
	...
	tmp11=(char *)malloc(sizeof(char)*100);
	bu=gtk_entry_get_text (GTK_ENTRY(entry13));
	bf=gtk_entry_get_text (GTK_ENTRY(create_contians_view_entry22));
	for(num=0;num<strlen(bu);num++){
	if(ispunct(bu[num]) || isspace(bu[num])){
	...
	sprintf(tmp11,"cat %s | sudo docker import - %s:%s",(char *)data,bu,bf);
		//g_printf("%s\n",contains_name_import_tag);
		//g_printf("%s666666666666666\n",tmp11);
		if(system(tmp11)==0)
	{
...
void import_create_file_view(gchar *buf)//导入功能填写界面
{
	...
	fram22=GTK_WIDGET(gtk_builder_get_object (builder,"frame1"));
	box11=GTK_WIDGET(gtk_builder_get_object (builder,"box2"));
	//gtk_widget_destroy (combox_images_import);
	gtk_widget_destroy (fram22);
	gtk_box_pack_start (GTK_BOX(box11),entry13,FALSE,FALSE,0);
	create_contians_view_entry22=GTK_WIDGET(gtk_builder_get_object (builder,"entry2"));
	button11=GTK_WIDGET(gtk_builder_get_object (builder,"button1"));
...
void import_print_file_name(GtkWidget *window)//获取导入功能选择文件的名字
{
	...
	import_file_name=gtk_file_chooser_get_filename (GTK_FILE_CHOOSER(window));
...
void import_selcet_file(GtkWidget *window,gpointer data)//创建导入文件的选择界面
{
	...
	dialog=gtk_file_chooser_dialog_new(_("please select import files"),GTK_WINDOW(window),GTK_FILE_CHOOSER_ACTION_OPEN,_("取消"),GTK_RESPONSE_CANCEL,_("sure"),
	                                   GTK_RESPONSE_OK,NULL);
	if(gtk_dialog_run (GTK_DIALOG(dialog))==GTK_RESPONSE_OK){
		//g_printf("oooooo\n");
		import_print_file_name (dialog);
...
void create_view(GtkWidget *container,gpointer data)//容器管理的主界面
{
	...
	gtk_window_set_title (GTK_WINDOW(window),_("contians manage"));
	...
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(column22),_("Mirror image"));
	cell=gtk_cell_renderer_text_new ();
	gtk_tree_view_column_pack_start (GTK_TREE_VIEW_COLUMN(column22), cell,FALSE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column22),cell,"text",0,NULL);
	
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(column33),"ID");
	cell=gtk_cell_renderer_text_new ();
	gtk_tree_view_column_pack_start (GTK_TREE_VIEW_COLUMN(column33),cell,FALSE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column33),cell,"text",1,NULL);
	
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(column44),_("name_colum"));
	cell=gtk_cell_renderer_text_new ();
	gtk_tree_view_column_pack_start (GTK_TREE_VIEW_COLUMN(column44),cell,FALSE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column44),cell,"text",2,NULL);
