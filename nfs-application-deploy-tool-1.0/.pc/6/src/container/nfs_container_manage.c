...
{
	GtkWidget *dialog;
	dialog=gtk_message_dialog_new(NULL,GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,type,GTK_BUTTONS_OK,(gchar *)buf);
...
void import_create_file_veiw_fun(GtkWidget *window,gpointer data)
{
	...
	if(contains_name_import_tag[0]==0){
	contains_message(GTK_MESSAGE_ERROR,_("The image name cannot is spaces"));
		return;
	}
	for(num22=0;num22<strlen(bf);num22++){
	...
	sprintf(tmp11,"cat %s | sudo docker import - %s:%s",(char *)data,contains_name_import_tag,bf);
