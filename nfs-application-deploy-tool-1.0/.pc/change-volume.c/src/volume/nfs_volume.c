...
#define BACKUP_VOLUME_FILE "src/ui/nfs_volume_backup.ui"
#define VOLUME_FILE "src/ui/nfs_volume.ui"
#define CREATE_VOLUMN_FILE "src/ui/nfs_volume_create.ui"
#define CREATE_CONTAINER_FILE "src/ui/nfs_volume_container_create.ui"
#define LOOK_VOLUME_FILE  "src/ui/nfs_volume_look.ui"
...
void data_volume_backup_pick_up_data(gchar *str, gchar *data, gint flag)
{
	...
	{
		sprintf(command, "echo %s | awk -F : '{for(i=1;i<NF;i++) printf $i\" \"}'"
	    			             "| awk '{for(i=1;i<=NF;i++){if(i%2==1){printf $i\" \"}}}'", data_tmp);
	}
