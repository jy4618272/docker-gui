...
static char my_link_contains_do[100]={0};//链接容器的动作参数
 ...
 static struct docker_images view_link[1024]={};
 struct docker_contains_link{
		char name[100];
		char  ID[100];
};

 struct docker_contains_link contians_link[1024]={};
void combox22_change_fun(GtkWidget *window,gpointer data)
{
	...
	model_link=gtk_combo_box_get_model (GTK_COMBO_BOX(window));
	gtk_combo_box_get_active_iter(GTK_COMBO_BOX(window), &iter_link);
...
void combox33_change_fun(GtkWidget *window,gpointer data)
{
		...
		model_c=gtk_combo_box_get_model (GTK_COMBO_BOX(window));
		gtk_combo_box_get_active_iter (GTK_COMBO_BOX(window),&iter_c);
...
gpointer link_contains_fun(gpointer data)
{
...
void button_link_fun(GtkWidget *window,gpointer data)
{
	...
	p=gtk_entry_get_text (GTK_ENTRY(contains_link_entry11));
	tmp=gtk_entry_get_text (GTK_ENTRY(contains_link_entry22));
	tmp1=gtk_entry_get_text (GTK_ENTRY(contains_link_entry33));
	...
	//printf("%s %s\n",link_image_tag,link_contains_id);
		//printf("%s\n",tmp11);
	g_thread_new ("c_link",link_contains_fun,NULL);
...
void create_link_contains()
{
	...
	gtk_window_set_title (GTK_WINDOW(window),_("contain connect"));
	...
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combox22), renderer,FALSE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combox22), renderer,
                             "text",0,
                               NULL);
	renderer11 = gtk_cell_renderer_text_new();
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combox22), renderer11,TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combox22), renderer11,
                             "text",1,
                               NULL);
	...
	while(fgets(buf_link,1024,fd_link))
{
	...
	gtk_list_store_set(store, &iter,0, view_link[link_nm].name,1,view_link[link_nm].tag,-1);
		...
		gtk_combo_box_set_model(GTK_COMBO_BOX(combox22),GTK_TREE_MODEL(store));
/*........................................................................................*/
	...
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(combox33),cell_contains,TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combox33),cell_contains,
                             "text",0,
                               NULL);
        GtkCellRenderer *cell_contains11=gtk_cell_renderer_text_new ();
        gtk_cell_layout_pack_start (GTK_CELL_LAYOUT(combox33),cell_contains11,TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combox33),cell_contains11,
                             "text",1,
                               NULL);
//gtk_combo_box_set_model (combox33,list_contains);
		...
		gtk_combo_box_set_model (GTK_COMBO_BOX(combox33),GTK_TREE_MODEL(list_contains));
		//int i;
	/*for(i=0;i<4;i++){
	gtk_list_store_append(list_contains, &iter);
