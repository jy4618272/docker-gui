...
void textbuffer_insert_fun(gpointer data)
{
		usleep(1000);
		gdk_threads_enter ();
...
void file_chooser_button22(GtkWidget *window,gpointer data)//选择Dockerfile文件确定按钮的信号
{
	...
	filename_chosser_file=gtk_file_chooser_get_filename (dialog11);
	printf("1111%s\n",filename_chosser_file);
	tmp=(char *)malloc(sizeof(filename_chooser)*50);
	tmp11=(char *)malloc(sizeof(filename_chooser)*50);
	//printf("%s\n",buf_filechooser);
	//printf("%s\n",filename_chooser);
	...
	message_file=gtk_message_dialog_new(dialog11,GTK_DIALOG_DESTROY_WITH_PARENT,
	                                    GTK_MESSAGE_INFO,GTK_BUTTONS_OK_CANCEL,tmp11);
		result_file=gtk_dialog_run (message_file);
...
gint real_save(char *data,char *buf)
{
       static FILE *fp=NULL;
        ...
        //fputs(buf,fp);
		gtk_window_set_title (window_space,data);
        fclose(fp);
	return 0;
...
void menuitem22_open(GtkWidget *window,gpointer data_open)//打开文件
{
	...
	contains_message (GTK_MESSAGE_ERROR,_("Failed to open file"));
		...
		gtk_window_set_title (window_space,filename_open);
		printf("titile is %s\n",gtk_window_get_title (window_space));
		g_free(buff);
		fclose (fb_open);
	//gtk_widget_destroy (dialog);
	}
...
GtkWidget *dialog_quit;
	...
	if(file_chang==0){
		//contains_message (GTK_MESSAGE_WARNING,"SSSSSS");
			...
			result_quit=gtk_dialog_run (dialog_quit);
...
void menuitem33_save(GtkWidget *window,gpointer data)//保存
{
	//printf("save\n");
		...
		buf_save=gtk_window_get_title (window_space);
        buffer=gtk_text_view_get_buffer(text_view_save);
        gtk_text_buffer_get_start_iter(buffer,&start);
        gtk_text_buffer_get_end_iter(buffer,&end);
        buf_save=gtk_text_buffer_get_text(buffer,&start,&end,TRUE);
			
	if(gtk_window_get_title (window_space) ==NULL)
	{
				//menuitem4_sav(window,text_view_save);	
				 create_file_chooser(buf_save);
				file_chang=1;

	}
		if(gtk_window_get_title (window_space) != NULL)
	{
			path_name=gtk_window_get_title (window_space);
			real_save(path_name,buf_save);
			file_chang=1;
			printf("%s\n",buf_save);
...
void curror_marr(GtkTextBuffer *buffer_mar,gpointer data)//光标的移动的信号函数
{
	...
	gtk_statusbar_push (status11,0,msg);
...
void button_next_fun(GtkWidget *window,gpointer *data)//查找下一个的按钮信号函数
	{
		...
		buffer23=gtk_entry_get_text (entry_find);
		
		
		buffer_find=gtk_text_view_get_buffer (text_view_save);
...
void menu_find_fun_quit(GtkWidget *window,gpointer data)//查找退出隐藏窗口
{
	...
	GtkTextIter start_iter_quit,end_iter_quit;
		...
		buffer_quit=gtk_text_view_get_buffer (text_view_save);
